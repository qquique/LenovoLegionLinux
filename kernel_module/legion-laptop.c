...
static const struct ec_register_offsets ec_register_offsets_loq_v0 = {
	...
	.EXT_FAN2_BASE = 0xC530, // gpu fan
	.EXT_FAN_ACC_BASE = 0xC5a0, // not found yet
	.EXT_FAN_DEC_BASE = 0xC5a0, // not found yet
	.EXT_CPU_TEMP = 0xC508, // cpu temp max
	.EXT_CPU_TEMP_HYST = 0xC507, // cpu temp min
	.EXT_GPU_TEMP = 0xC52F, // gpu temp max
	.EXT_GPU_TEMP_HYST = 0xC52E, // gpu temp min
...
static const struct ec_register_offsets ec_register_offsets_loq_v1 = {
	...
	.EXT_CPU_TEMP = 0xC508, // cpu temp max
	.EXT_CPU_TEMP_HYST = 0xC507, // cpu temp min
	.EXT_GPU_TEMP = 0xC53B, // gpu temp max
	.EXT_GPU_TEMP_HYST = 0xC53A, // gpu temp min
	.EXT_VRM_TEMP = 0xC5a0, // not found yet
	.EXT_VRM_TEMP_HYST = 0xC5a0, // not found yet
	.EXT_FAN1_RPM_LSB = 0xC5a0, // not found yet
	.EXT_FAN1_RPM_MSB = 0xC5a0, // not found yet
...
static int wmi_exec_query_ints(const char *guid, u8 instance, u8 *res, size_t ressize)
{
	acpi_status status;
	struct acpi_buffer out_buffer = { ACPI_ALLOCATE_BUFFER, NULL };
...
struct WMIFanTableReadLoq {
	...
	u8  SOU1;
	u8  SOU2;
	u16 CFMS; // Current Fan Max Speed
	u8  SOU3; // Design Fan Max Speed Number
	u8  SOU4; // Current Fan Min Speed
...
static int auto_points_defaults = 0;
					...
					struct fancurve *fancurve)
{
	...
	u8* buffer = (u8*)&fantable;
  int powermode_index = 0;

  switch (auto_points_defaults) {
		case 0x1: 
			powermode_index = LOQ_FANTABLE_QUIET_INDEX;
			break;
		case 0x3:
			powermode_index = LOQ_FANTABLE_PERFORMANCE_INDEX;
			break;
		case 0x2:
			powermode_index = LOQ_FANTABLE_BALANCED_INDEX;
			break;
		case 0xFF:
			powermode_index = LOQ_FANTABLE_CUSTOM_INDEX;
			break;
		case 0xE0:
			powermode_index = LOQ_FANTABLE_EXTREME_INDEX;
			break;
		default:
			// dont show anything until auto_points_defaults is set ( != 0)
			return 0;
	}

	// Get CPU values
	err = wmi_exec_query_ints("87FB2A6D-D802-48E7-9208-4576C5F5C8D8", 
						powermode_index, buffer, sizeof(struct WMIFanTableReadLoq));
	if (!err) {
		fancurve->current_point_i = 0;
		...
		print_hex_dump(KERN_DEBUG, "legion_laptop fan table idx wmi buffer",
		       DUMP_PREFIX_ADDRESS, 16, 1, buffer, sizeof(struct WMIFanTableReadLoq),
		       true);
	}

	// Get GPU values
	err = wmi_exec_query_ints("87FB2A6D-D802-48E7-9208-4576C5F5C8D8", 
						powermode_index + 1, buffer, sizeof(struct WMIFanTableReadLoq));
	if (!err) {
	...
	// Get IC values ???
	err = wmi_exec_query_ints("87FB2A6D-D802-48E7-9208-4576C5F5C8D8", 
						powermode_index + 2, buffer, sizeof(struct WMIFanTableReadLoq));
	if (!err) {
				...
				struct fancurve *fancurve)
{
	...
	for (i = 0; i < FANCURVESIZE_LOQ; ++i) {
		...
		// gpu temps are the same as cpu on this ec memory section,
		// internally are correctly applied and used from acpi fan tables
	...
	for (i = 0; i < FANCURVESIZE_LOQ; ++i) {
		...
		ecram_write(ecram,
			    model->registers->EXT_FAN2_BASE +
				    (i * struct_offset),
			    point->speed1);
		valr1 = ecram_read(ecram, model->registers->EXT_FAN2_BASE +
						  (i * struct_offset));
		...
		ecram_write(ecram,
			    model->registers->EXT_CPU_TEMP_HYST +
				    (i * struct_offset),
			    point->cpu_min_temp_celsius);
		// no need to write the gpu temps
	}
...
static int debugfs_fancurve_show(struct seq_file *s, void *unused)
{
	...
	err = wmi_read_fancurve_idx(priv->conf, &wmi_fancurve);
				  ...
				  const char *buf, size_t count)
{
	...
	if (err) {
		err = -1;
		pr_info("Parsing auto_points_defaults store failed: error:%d\n", err);
		goto error;
	}
	...
	if (!(value == LEGION_WMI_POWERMODE_QUIET ||
		...
		err = -1;
		pr_info("Parsing auto_points_defaults store failed invalid powermode id %d: error:%d\n", value, err);
		goto error;
	}
